...
#define SWITCHER_PIN_CAMERA     (15)    // D8 (boot fails if pulled high, therefore only connected to a BJT base via 10k resistor)

#define SWITCHER_PIN_HEATER1    (2)     // D4 (internal pull up, boot fails if pulled low, therefore only connected to a MOSFET gate via 2k + 10k resistor)
#define SWITCHER_PIN_HEATER2    (0)     // D3 (same as D4)
